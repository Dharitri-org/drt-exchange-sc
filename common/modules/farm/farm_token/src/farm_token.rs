    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn burn_farm_tokens_from_payments(&self, payments: &ManagedVec<DctTokenPayment<Self::Api>>) {
        let mut total_amount = BigUint::zero();
        for entry in payments.iter() {
            total_amount += &entry.amount;
            self.send()
                .dct_local_burn(&entry.token_identifier, entry.token_nonce, &entry.amount);
        }
    ...
    fn mint_farm_tokens<T: TopEncode>(
        ...
        let new_nonce = self
            .send()
            .dct_nft_create_compact(&token_id, &amount, attributes);
        self.farm_token_supply().update(|x| *x += &amount);

        DctTokenPayment::new(token_id, new_nonce, amount)
    }
    ...
    fn burn_farm_token_payment(&self, payment: &DctTokenPayment<Self::Api>) {
