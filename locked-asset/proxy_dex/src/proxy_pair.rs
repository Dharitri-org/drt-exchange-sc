    ...
    + legacy_token_decode_module::LegacyTokenDecodeModule
{
    ...
    ) -> MultiValueEncoded<DctTokenPayment> {
    ...
    fn remove_liquidity_proxy(
        ...
        if base_asset_amount_received > locked_token_amount_available {
            ...
            let unlocked_tokens = DctTokenPayment::new(asset_token_id.clone(), 0, unlocked_amount);

            // burn base asset, as we only need to send the locked tokens
            self.send()
                .dct_local_burn(&asset_token_id, 0, &attributes.locked_tokens.amount);
            ...
            let asset_token_id = received_token_refs
                ...
                .clone();
            self.send()
                .dct_local_burn(&asset_token_id, 0, &locked_tokens_out.amount);
