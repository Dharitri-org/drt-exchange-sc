...
pub struct EnterFarmResult<M: ManagedTypeApi> {
    pub farming_token: DctTokenPayment<M>,
    pub farm_token: DctTokenPayment<M>,
    pub rewards: DctTokenPayment<M>,
}

pub type EnterFarmProxyResultType<M> = MultiValue2<DctTokenPayment<M>, DctTokenPayment<M>>;
pub type ExitFarmProxyResultType<M> = MultiValue2<DctTokenPayment<M>, DctTokenPayment<M>>;
pub type ClaimRewardsFarmProxyResultType<M> = MultiValue2<DctTokenPayment<M>, DctTokenPayment<M>>;
    ...
    + sc_whitelist_module::SCWhitelistModule
{
    ...
    fn enter_farm_locked_token(
        ...
        user: ManagedAddress,
        farm_address: ManagedAddress,
        locked_token: DctTokenPayment,
    ) -> EnterFarmResult<Self::Api> {
        let asset_token_id = self.get_base_token_id();
        self.send()
            .dct_local_mint(&asset_token_id, 0, &locked_token.amount);

        let minted_asset_tokens = DctTokenPayment::new(asset_token_id, 0, locked_token.amount);
    ...
    fn enter_farm_wrapped_lp(
        ...
        user: ManagedAddress,
        farm_address: ManagedAddress,
        wrapped_lp_token: DctTokenPayment,
    ) -> EnterFarmResult<Self::Api> {
        ...
        let farming_token = DctTokenPayment::new(
    ...
    fn increase_proxy_farm_token_energy_endpoint(&self, lock_epochs: Epoch) -> DctTokenPayment {
        self.require_wrapped_farm_token_id_not_empty();
        self.require_wrapped_lp_token_id_not_empty();
