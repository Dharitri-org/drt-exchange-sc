...
use dharitri_sc::{
    ...
    types::{BigInt, DctLocalRole, DctTokenPayment},
};
...
fn farm_with_wrapped_lp_test() {
    ...
    setup.b_mock.set_dct_local_roles(
        setup.farm_locked_wrapper.address_ref(),
        LP_TOKEN_ID,
        &[DctLocalRole::Burn],
    );
    ...
    setup.b_mock.check_nft_balance(
        ...
        &(&expected_lp_token_amount / 2u64 - &penalty_amount),
        Some(&WrappedLpTokenAttributes::<DebugApi> {
            locked_tokens: DctTokenPayment::new(
...
fn different_farm_locked_token_nonce_merging_test() {
    ...
    setup
        .b_mock
        .set_dct_balance(&first_user, MEX_TOKEN_ID, &user_balance);
