...
deploySimpleLockContract() {
    ...
    ADDRESS=$(moapy data parse --file="deploy-simple-lock.interaction.json" --expression="data['emitted_tx']['address']")
...
upgradeSimpleLockContract() {
    moapy --verbose contract upgrade $SIMPLE_LOCK_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=150000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --metadata-payable \
...
issueLockedToken() {
    ...
    moapy --verbose contract call $SIMPLE_LOCK_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=100000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
addLpToWhitelist() {
    ...
    pair_address="0x$(moapy wallet bech32 --decode $PAIR_ADDRESS)"
