    ...
    + utils::UtilsModule
{
    ...
    fn set_transfer_role(&self, opt_address: OptionalValue<ManagedAddress>) {
        ...
        self.locked_token()
            .set_local_roles_for_address(&address, &[DctLocalRole::Transfer], None);
    }
    ...
    fn set_locked_token(&self, token_id: TokenIdentifier) {
        require!(token_id.is_valid_dct_identifier(), "Token id is not valid");
    ...
    fn lock_tokens_endpoint(
        ...
        self.lock_and_send(
            &dest_address,
            MoaxOrDctTokenPayment::from(payment),
            unlock_epoch,
        )
    }
