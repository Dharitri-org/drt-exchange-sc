...
use dharitri_wasm::types::{DctLocalRole, MultiValueEncoded};
use dharitri_wasm_debug::{managed_token_id, rust_biguint, DebugApi};
use dharitri_wasm_debug::{managed_token_id_wrapped, testing_framework::*};
...
fn lock_whitelist_test() {
    ...
    b_mock.set_dct_balance(&user_addr, FREE_TOKEN_ID, &lock_amount);
    b_mock.set_dct_balance(&user_addr, OTHER_TOKEN_ID, &lock_amount);
