...
fn token_merging_test() {
    ...
    setup
        .b_mock
        .execute_dct_multi_transfer(&first_user, &setup.sc_wrapper, &payments[..], |sc| {
...
fn test_specific_tokens_merge() {
    ...
    b_mock
        .execute_tx(&owner, &sc_wrapper, &rust_zero, |sc| {
            ...
            let _ = sc.lock_and_send(
                ...
                MoaxOrDctTokenPayment::new(
                    MoaxOrDctTokenIdentifier::dct(managed_token_id!(BASE_ASSET_TOKEN_ID)),
                    0,
    ...
    b_mock
        .execute_dct_multi_transfer(&user, &sc_wrapper, &payments, |sc| {
            let _ = sc.merge_tokens_endpoint(OptionalValue::None);
        })
        .assert_ok();
