    ...
    + legacy_token_decode_module::LegacyTokenDecodeModule
{
    ...
    ///     NOTE: The SC also needs the DCTLocalMint and DCTLocalBurn roles for this token.
    ...
    /// Locks a whitelisted token until `unlock_epoch` and receive meta DCT LOCKED tokens
    ...
    fn unlock_tokens_endpoint(&self) -> DctTokenPayment {
        self.require_not_paused();
        ...
        let mut output_payment = DctTokenPayment::new(base_asset, 0, BigUint::zero());
        ...
        self.update_energy(&caller, |energy: &mut Energy<Self::Api>| {
            ...
            for payment in &payments {
                ...
                output_payment.merge_with(self.to_dct_payment(unlocked_tokens));
            }
        });

        self.send()
            .dct_local_mint(&output_payment.token_identifier, 0, &output_payment.amount);
        self.send().direct_dct(
            &caller,
    ...
    fn extend_lock_period(&self, lock_epochs: Epoch, user: ManagedAddress) -> DctTokenPayment {
        self.require_not_paused();
        self.require_is_listed_lock_option(lock_epochs);
