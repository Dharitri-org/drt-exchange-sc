    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn set_extended_attributes_activation_nonce(&self, is_sc_upgrade: bool) {
        ...
        if is_sc_upgrade {
            ...
            let current_nonce = self
                .blockchain()
                .get_current_dct_nft_nonce(&own_sc_address, &token_id);
    ...
    fn unlock_assets(&self) {
        ...
        let (token_id, token_nonce, amount) = self.call_value().single_dct().into_tuple();
        ...
        let mut output_locked_assets_token_amount =
            DctTokenPayment::new(token_id.clone(), 0, BigUint::zero());
        ...
        self.send()
            .dct_local_burn(&locked_token_id, token_nonce, &amount);
    ...
    #[payable("*")]
    #[endpoint(lockAssets)]
    fn lock_assets(&self) -> DctTokenPayment<Self::Api> {
        self.require_not_paused();
        ...
        self.send()
            .dct_local_burn(&payment_token, 0, &payment_amount);
