    ...
    + utils::UtilsModule
{
    ...
    fn stake_farm_through_proxy(&self, staked_token_amount: BigUint) -> DctTokenPayment {
        let caller = self.blockchain().get_caller();
        self.require_sc_address_whitelisted(&caller);

        let staked_token_id = self.farming_token_id().get();
        let staked_token_simulated_payment =
            DctTokenPayment::new(staked_token_id, 0, staked_token_amount);

        let farm_tokens = self.call_value().all_dct_transfers();
    ...
    #[payable("*")]
    #[endpoint(stakeFarm)]
    fn stake_farm_endpoint(&self) -> DctTokenPayment {
        let payments = self.get_non_empty_payments();

        self.stake_farm_common(payments)
    }

    fn stake_farm_common(&self, payments: PaymentsVec<Self::Api>) -> DctTokenPayment {
        let caller = self.blockchain().get_caller();
        let enter_result =
