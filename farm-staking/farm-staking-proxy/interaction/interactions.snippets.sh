...
deployProxyStakeFarmContract() {
    lp_farm_address="0x$(moapy wallet bech32 --decode $1)"
    staking_farm_address="0x$(moapy wallet bech32 --decode $2)"
    pair_address="0x$(moapy wallet bech32 --decode $3)"
    ...
    ADDRESS=$(moapy data parse --file="deploy-proxy-stake-farm-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeProxyStakeFarmContract() {
    ...
    moapy --verbose contract upgrade $8 --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=200000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --metadata-payable \
