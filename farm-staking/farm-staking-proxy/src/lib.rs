...
pub type StakeResult<Api> = DctTokenPayment<Api>;
pub type ClaimDualYieldResult<Api> = MultiValueEncoded<Api, DctTokenPayment<Api>>;
pub type UnstakeResult<Api> = MultiValueEncoded<Api, DctTokenPayment<Api>>;
    ...
    + dharitri_wasm_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn init(
        ...
        require!(
            staking_token_id.is_valid_dct_identifier(),
            "Invalid Staking token ID"
        );
        require!(
            lp_farm_token_id.is_valid_dct_identifier(),
            "Invalid Farm token ID"
        );
        require!(
            staking_farm_token_id.is_valid_dct_identifier(),
            "Invalid Staking Farm token ID"
        );

        require!(
            lp_token_id.is_valid_dct_identifier(),
    ...
    fn stake_farm_tokens(&self) -> StakeResult<Self::Api> {
        ...
        let lp_farm_token_payment: DctTokenPayment<Self::Api> = payments
            .try_get(0)
    ...
    fn claim_dual_yield(&self) -> ClaimDualYieldResult<Self::Api> {
        let (payment_token, payment_nonce, payment_amount) =
            self.call_value().single_dct().into_tuple();
    ...
    fn send_claim_payments(
        &self,
        lp_farm_rewards: DctTokenPayment<Self::Api>,
        staking_farm_rewards: DctTokenPayment<Self::Api>,
        new_dual_yield_tokens: DctTokenPayment<Self::Api>,
    ...
    fn send_unstake_payments(
        &self,
        other_token_payment: DctTokenPayment<Self::Api>,
        lp_farm_rewards: DctTokenPayment<Self::Api>,
        staking_rewards: DctTokenPayment<Self::Api>,
        unbond_staking_farm_token: DctTokenPayment<Self::Api>,
    ) -> UnstakeResult<Self::Api> {
        let caller = self.blockchain().get_caller();
