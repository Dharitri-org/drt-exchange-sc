    ...
    + enable_swap_by_user::EnableSwapByUserModule
{
    ...
    fn create_pair_endpoint(
        ...
        require!(
            first_token_id.is_valid_dct_identifier(),
            "First Token ID is not a valid dct token ID"
        );
        require!(
            second_token_id.is_valid_dct_identifier(),
            "Second Token ID is not a valid dct token ID"
        );
    ...
    fn issue_lp_token(
        ...
        let result: TokenIdentifier = self
            ...
            .execute_on_dest_context();
        require!(
            !result.is_valid_dct_identifier(),
    ...
    fn set_local_roles(&self, pair_address: ManagedAddress) {
        ...
        let pair_token: TokenIdentifier = self
            ...
            .get_lp_token_identifier()
            .execute_on_dest_context();
        require!(pair_token.is_valid_dct_identifier(), "LP token not issued");
    ...
    fn lp_token_issue_callback(
        ...
        let (token_id, returned_tokens) = self.call_value().moax_or_single_fungible_dct();
        match result {
            ManagedAsyncCallResult::Ok(()) => {
                self.pair_temporary_owner().remove(address);
                let _: IgnoreValue = self
                    .pair_contract_proxy(address.clone())
                    .set_lp_token_identifier(token_id.unwrap_dct())
                    .execute_on_dest_context();
            }
            ManagedAsyncCallResult::Err(_) => {
                if token_id.is_moax() && returned_tokens > 0u64 {
