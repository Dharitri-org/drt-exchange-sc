...
pub trait MultiPairSwap: factory::FactoryModule + token_send::TokenSendModule {
    ...
    fn multi_pair_swap(&self, swap_operations: MultiValueEncoded<SwapOperationType<Self::Api>>) {
        let (token_id, nonce, amount) = self.call_value().single_dct().into_tuple();
        ...
        let mut last_payment = DctTokenPayment::new(token_id, nonce, amount);
    ...
    fn actual_swap_fixed_output(
        ...
        token_out: TokenIdentifier,
        amount_out: BigUint,
    ) -> (DctTokenPayment<Self::Api>, DctTokenPayment<Self::Api>) {
