...
issueToken() {
    ...
    moapy --verbose contract call ${DCT_ISSUE_ADDRESS} --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=60000000 \
...
deployRouterContract() {
    ...
    ADDRESS=$(moapy data parse --file="deploy-route-internal.interaction.json" --expression="data['contractAddress']")

    moapy data store --key=router-address --value=${ADDRESS}
...
upgradeRouterContract() {
    moapy --verbose contract upgrade ${ROUTE_ADDRESS} --recall-nonce \
          --pem=${WALLET_PEM} \
...
uploadPairContractCode() {
    ...
    moapy --verbose contract call ${ROUTE_ADDRESS} --recall-nonce \
          --pem=${WALLET_PEM} \
          --proxy=${PROXY} --chain=${CHAIN_ID} \
...
createPair() {
    ...
    moapy --verbose contract query ${ROUTE_ADDRESS} \
    --proxy=${PROXY} \
    --function=getPair \
...
setFeeOn() {
    pair_address="0x$(moapy wallet bech32 --decode $1)"
    farm_contract="0x$(moapy wallet bech32 --decode $2)"
...
deployPairContract() {
    first_token="0x$(echo -n $1 | xxd -p -u | tr -d '\n')"
    second_token="0x$(echo -n $2 | xxd -p -u | tr -d '\n')"
    router_address="0x$(moapy wallet bech32 --decode $ROUTE_ADDRESS)"
    user_address="$(moapy wallet pem-address $WALLET_PEM)"
    user_address_decode="0x$(moapy wallet bech32 --decode $user_address)"
    ...
    ADDRESS=$(moapy data parse --file="deploy-pair-internal.interaction.json" --expression="data['contractAddress']")
...
upgradePairContract() {
    ...
    moapy --verbose contract upgrade $3 --recall-nonce \
          --pem=${WALLET_PEM} \
          --gas-limit=250000000 \
          --proxy=${PROXY} --chain=${CHAIN_ID} \
          --bytecode="../pair/output/pair.wasm" \
...
addLiquidity() {
    method_name="0x$(echo -n 'acceptDctPayment' | xxd -p -u | tr -d '\n')"

    moapy --verbose contract call $1 --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=30000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --function="multiDCTNFTTransfer" 2 $2 0 $3 $4 0 $5 ${method_name} $6 $7 \
        --arguments $2 $3 $4 $5 \
        --send || return
}
...
whitelist() {
    pair_address="0x$(moapy wallet bech32 --decode $2)"
    moapy --verbose contract call $1 --recall-nonce \
        --pem=${WALLET_PEM} \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
deployFarmContract() {
    ...
    locked_asset_factory_address="0x$(moapy wallet bech32 --decode $3)"
    ...
    ADDRESS=$(moapy data parse --file="deploy-farm-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeFarmContract() {
    ...
    locked_asset_factory_address="0x$(moapy wallet bech32 --decode $4)"
...
deployWMOAXContract() {
    ...
    ADDRESS=$(moapy data parse --file="deploy-wmoax-internal.interaction.json" --expression="data['contractAddress']")

    moapy data store --key=router-address --value=${ADDRESS}

    echo ""
    echo "WMOAX Smart contract address: ${ADDRESS}"
}

issueWrappedMoax() {
    moapy --verbose contract call ${WMOAX_WRAP_ADDRESS} --recall-nonce \
        --pem=${WALLET_PEM} \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --gas-limit=${DEPLOY_GAS} \
        --value=5000000000000000000 \
