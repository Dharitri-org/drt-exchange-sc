...
pub mod fuzz_data_tests {
    ...
    pub const OWNER_MOAX_BALANCE: u64 = 100_000_000;
    pub const USER_TOTAL_MEX_TOKENS: u64 = 100_000_000_000;
    pub const USER_TOTAL_WMOAX_TOKENS: u64 = 100_000_000_000;
        ...
        PriceDiscObjBuilder: 'static + Copy + Fn() -> price_discovery::ContractObj<DebugApi>,
    {
        pub fn new(
            ...
            let moax_amount = rust_biguint!(OWNER_MOAX_BALANCE);

            let rng = StdRng::seed_from_u64(seed);
            let fuzz_args = FuzzDexExecutorInitArgs::new();
            let statistics = EventsStatistics::new();
            let mut blockchain_wrapper = BlockchainStateWrapper::new();
            let owner_addr = blockchain_wrapper.create_user_account(&moax_amount);

            let mut users = vec![];

            for i in 1..=fuzz_args.num_users {
                let user_address = blockchain_wrapper.create_user_account(&moax_amount);
                blockchain_wrapper.set_dct_balance(
                    &user_address,
                    WMOAX_TOKEN_ID,
        ...
        FarmObjBuilder: 'static + Copy + Fn() -> farm::ContractObj<DebugApi>,
    {
        ...
        let farming_token_roles = [DctLocalRole::Mint, DctLocalRole::Burn];
        blockchain_wrapper.set_dct_local_roles(
            farm_wrapper.address_ref(),
            farming_token,
            &farming_token_roles[..],
        ...
        FactoryObjBuilder: 'static + Copy + Fn() -> factory::ContractObj<DebugApi>,
    {
        ...
        let token_roles = [DctLocalRole::Mint, DctLocalRole::Burn];
