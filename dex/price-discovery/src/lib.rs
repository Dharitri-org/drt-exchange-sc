    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn init(
        ...
        accepted_token_id: MoaxOrDctTokenIdentifier,
        ...
        require!(
            launched_token_id.is_valid_dct_identifier(),
    ...
    #[payable("*")]
    #[endpoint]
    fn deposit(&self) -> DctTokenPayment<Self::Api> {
    ...
    #[payable("*")]
    #[endpoint]
    fn withdraw(&self) -> MoaxOrDctTokenPayment<Self::Api> {
        let phase = self.get_current_phase();
        ...
        let (refund_token_id, balance_mapper) = match payment_nonce {
            LAUNCHED_TOKEN_REDEEM_NONCE => (
                MoaxOrDctTokenIdentifier::dct(self.launched_token_id().get()),
        ...
        MoaxOrDctTokenPayment::new(refund_token_id, 0, withdraw_amount)
    }
    ...
    #[payable("*")]
    #[endpoint]
    fn redeem(&self) -> MoaxOrDctTokenPayment<Self::Api> {
        let phase = self.get_current_phase();
    ...
    fn compute_bought_tokens(
    ...
    ) -> MoaxOrDctTokenPayment<Self::Api> {
        ...
        MoaxOrDctTokenPayment::new(token_id, 0, reward_amount)
    }
