...
use dharitri_wasm::{
    ...
    types::{DctLocalRole, MultiValueEncoded},
};
...
fn test_swap_fixed_input() {
    ...
    pair_setup.swap_fixed_input(WMOAX_TOKEN_ID, 1_000, MEX_TOKEN_ID, 900, 996);
}
...
fn test_swap_fixed_output() {
    ...
    pair_setup.swap_fixed_output(WMOAX_TOKEN_ID, 1_000, MEX_TOKEN_ID, 900, 96);
}
...
fn test_safe_price() {
    ...
    pair_setup.swap_fixed_input(WMOAX_TOKEN_ID, 1_000, MEX_TOKEN_ID, 900, 984);
    ...
    pair_setup.swap_fixed_input(WMOAX_TOKEN_ID, 1_000, MEX_TOKEN_ID, 900, 982);

    pair_setup.b_mock.set_block_nonce(90);
    pair_setup.swap_fixed_input(WMOAX_TOKEN_ID, 1_000, MEX_TOKEN_ID, 900, 980);

    pair_setup.b_mock.set_block_nonce(100);
    pair_setup.swap_fixed_input(WMOAX_TOKEN_ID, 1_000, MEX_TOKEN_ID, 900, 978);
    ...
    pair_setup.swap_fixed_input(WMOAX_TOKEN_ID, 1_000, MEX_TOKEN_ID, 900, 976);
...
fn test_locked_asset() {
    ...
    pair_setup.b_mock.set_dct_local_roles(
    ...
    pair_setup.b_mock.check_nft_balance(
        ...
        Some(&LockedTokenAttributes::<DebugApi> {
            original_token_id: managed_token_id_wrapped!(WMOAX_TOKEN_ID),
            original_token_nonce: 0,
            unlock_epoch: 10,
        }),
    );

    let user_wmoax_balance_before =
        pair_setup
            .b_mock
            .get_dct_balance(&pair_setup.user_address, WMOAX_TOKEN_ID, 0);
    ...
    pair_setup
        ...
        .assert_ok();
    pair_setup.b_mock.check_dct_balance(
        &pair_setup.user_address,
        WMOAX_TOKEN_ID,
        &(user_wmoax_balance_before + rust_biguint!(996)),
    );
}
...
fn add_liquidity_through_simple_lock_proxy() {
    ...
    // add liquidity through simple-lock SC - one locked (WMOAX) token, one unlocked (MEX)
    let transfers = vec![
        TxInputDCT {
    ...
    pair_setup.b_mock.check_nft_balance(
        ...
        Some(&LpProxyTokenAttributes::<DebugApi> {
            ...
            first_token_id: managed_token_id!(WMOAX_TOKEN_ID),
    ...
    let user_locked_token_balance_before =
        pair_setup
            .b_mock
            .get_dct_balance(&pair_setup.user_address, LOCKED_TOKEN_ID, 1);
    let user_mex_balance_before =
        pair_setup
            .b_mock
            .get_dct_balance(&pair_setup.user_address, MEX_TOKEN_ID, 0);
...
fn fees_collector_pair_test() {
    ...
    pair_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                tokens.push(managed_token_id!(WMOAX_TOKEN_ID));
    ...
    pair_setup.swap_fixed_input(WMOAX_TOKEN_ID, 100_000, MEX_TOKEN_ID, 900, 90_669);
    ...
    pair_setup
        ...
        .execute_query(&fees_collector_wrapper, |sc| {
            assert_eq!(
                sc.accumulated_fees(1, &managed_token_id!(WMOAX_TOKEN_ID))
                    .get(),
                managed_biguint!(25)
            );
        })
