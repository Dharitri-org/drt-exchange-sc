    ...
    FarmObjBuilder: 'static + Copy + Fn() -> farm::ContractObj<DebugApi>,
{
    ...
    pub fn enter_farm(&mut self, caller: &Address, farm_in_amount: RustBigUint) {
        ...
        b_mock
            .execute_dct_multi_transfer(caller, &self.farm_wrapper, &payments, |sc| {
    ...
    pub fn exit_farm(
        ...
        b_mock.check_dct_balance(caller, MEX_TOKEN_ID, &expected_mex_balance);
    }
    ...
    pub fn new_address_with_lp_tokens(&mut self, amount: RustBigUint) -> Address {
        ...
        blockchain_wrapper.set_dct_balance(&address, LP_TOKEN_ID, &amount);
        address
    }
}
